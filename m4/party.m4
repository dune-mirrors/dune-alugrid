...
PARTYDEF="-DDIM=$with_problem_dim -DDIM_OF_WORLD=$with_world_dim"

# set variables so that tests can use them
REM_CPPFLAGS=$CPPFLAGS

LDFLAGS="$LDFLAGS -L$PARTY_LIB_PATH"
CPPFLAGS="$CPPFLAGS $PARTYDEF -DEL_INDEX=0 -I$PARTY_INCLUDE_PATH"
...
CPPFLAGS="$REM_CPPFLAGS $PARTYDEF -I$PARTY_INCLUDE_PATH"
REM_CPPFLAGS=
...
if test x$HAVE_PARTY = x1 ; then
  ...
  # add to global list
  ALU3D_PKG_LDFLAGS="$ALU3D_PKG_LDFLAGS $PARTY_LDFLAGS"
  ALU3D_PKG_LIBS="$ALU3D_PKG_LIBS $PARTY_LIBS"
  ALU3D_PKG_CPPFLAGS="$ALU3D_PKG_CPPFLAGS $PARTY_CPPFLAGS"
