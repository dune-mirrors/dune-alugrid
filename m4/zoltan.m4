...
if test x$with_zoltan != xno ; then
  ...
  ZOLTAN_INC_FLAG="-I$ZOLTAN_INCLUDE_PATH -I$ZOLTAN_INCLUDE_PATH/serial -I$ZOLTAN_INCLUDE_PATH/duneinterface -DENABLE_ZOLTAN"
  CPPFLAGS="$ac_save_CPPFLAGS $ZOLTAN_INC_FLAG"
  # check for header
  ...
  ac_save_LDFLAGS="$LDFLAGS"
  LDFLAGS="$LDFLAGS -L$ZOLTAN_LIB_PATH"
  ac_save_LIBS="$LIBS"
